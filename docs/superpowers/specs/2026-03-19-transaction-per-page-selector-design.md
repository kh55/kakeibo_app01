...
- `(int) $request->get('per_page', 50)` で件数を取得（文字列→int キャスト。コントローラー既存の year/month と同パターン）
- 許可値 `[20, 50, 100]` に含まれない場合は 50 にフォールバック（`in_array` 非厳密モード）
...
- レイアウト: 現在4列すべて `col-md-3` で埋まっているため、year・month・type・per_page を `col-md-2` に縮小し、フィルタボタンを `col-md-auto` に変更して5列構成にする
