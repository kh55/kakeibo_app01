...
class InstallmentPlanController extends Controller
{
    ...
    public function index()
    {
        ...
        $recordedThisMonth = Transaction::where('user_id', $user->id)
            ->whereNotNull('installment_plan_id')
            ->whereYear('date', now()->year)
            ->whereMonth('date', now()->month)
            ->pluck('installment_plan_id')
            ->toArray();

        return view('installments.index', compact('plans', 'recordedThisMonth'));
    }
     ...
     */
    public function recordPayment(InstallmentPlan $installmentPlan)
    {
        $this->authorize('update', $installmentPlan);
        ...
        $alreadyRecorded = Transaction::where('user_id', Auth::id())
        ...
        if ($alreadyRecorded) {
            return redirect()->route('installment-plans.index')
                ->with('error', '今月はすでに支払いを記録済みです。');
        }

        $payDay = min($installmentPlan->pay_day, now()->daysInMonth);
