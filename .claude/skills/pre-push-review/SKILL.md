...
## Step 2: 自動チェック（PHPUnit + Pint + Playwright）
...
- Playwright の失敗は `npx playwright test --headed` でブラウザを表示して原因を確認できる
- Playwright 実行前に Docker が起動済みであること（`docker compose up -d`）
